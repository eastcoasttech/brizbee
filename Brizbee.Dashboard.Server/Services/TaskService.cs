...
namespace Brizbee.Dashboard.Server.Services
{
    public class TaskService
    {
        ...
        public async Task<(List<Core.Models.Task>, long?)> GetTasksAsync(int jobId, int pageSize = 100, int skip = 0, string sortBy = "Number", string sortDirection = "ASC")
        {
            ...
            var odataResponse = await JsonSerializer.DeserializeAsync<ODataListResponse<Core.Models.Task>>(responseContent, options);
        ...
        public async Task<Core.Models.Task> GetTaskByIdAsync(int id)
        {
            ...
            using var responseContent = await response.Content.ReadAsStreamAsync();
            return await JsonSerializer.DeserializeAsync<Core.Models.Task>(responseContent);
        }

        public async Task<(List<Core.Models.Task>, long?)> GetTasksForPunchesAsync(DateTime min, DateTime max)
        {
            ...
            if (!response.IsSuccessStatusCode)
                return (new List<Core.Models.Task>(0), 0);

            using var responseContent = await response.Content.ReadAsStreamAsync();
            var value = await JsonSerializer.DeserializeAsync<List<Core.Models.Task>>(responseContent, options);
            var total = long.Parse(response.Headers.GetValues("X-Paging-TotalRecordCount").FirstOrDefault());
            return (value, total);
        }

        public async Task<Core.Models.Task> SearchTasksAsync(string number)
        {
            ...
            {
                using var responseContent = await response.Content.ReadAsStreamAsync();
                var odataResponse = await JsonSerializer.DeserializeAsync<Core.Models.Task>(responseContent, options);
        ...
        public async Task<Core.Models.Task> SaveTaskAsync(Core.Models.Task task)
        {
                ...
                using (var stringContent = new StringContent(json, Encoding.UTF8, "application/json"))
                {
                        ...
                        .ConfigureAwait(false))
                    {
                        if (response.IsSuccessStatusCode)
                        {
                            ...
                            {
                                var deserialized = await JsonSerializer.DeserializeAsync<Core.Models.Task>(responseContent, options);
                                return deserialized;
                            }
                        }
                        else
