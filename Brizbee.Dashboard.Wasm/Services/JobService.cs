...
namespace Brizbee.Dashboard.Wasm.Services
{
    public class JobService
    {
        ...
        public async Task<(List<Job>, long?)> GetExpandedJobsAsync(int pageSize = 100, int skip = 0, string sortBy = "JOBS/NUMBER", string sortDirection = "ASC", string filterStatus = "Open")
        {
            var response = await _apiService.GetHttpClient().GetAsync($"api/JobsExpanded?pageSize={pageSize}&skip={skip}&orderBy={sortBy}&orderByDirection={sortDirection}&jobStatuses={filterStatus}");
