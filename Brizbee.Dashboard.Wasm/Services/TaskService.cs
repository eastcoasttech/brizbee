...
namespace Brizbee.Dashboard.Wasm.Services
{
    public class TaskService
    {
        ...
        public async Task<(List<Brizbee.Dashboard.Wasm.Models.Task>, long?)> GetTasksAsync(int jobId, int pageSize = 100, int skip = 0, string sortBy = "Number", string sortDirection = "ASC")
        {
            ...
            var odataResponse = await JsonSerializer.DeserializeAsync<ODataListResponse<Brizbee.Dashboard.Wasm.Models.Task>>(responseContent, options);
            return (odataResponse.Value.ToList(), odataResponse.Count);
        }

        public async Task<Brizbee.Dashboard.Wasm.Models.Task> GetTaskByIdAsync(int id)
        {
            var response = await _apiService.GetHttpClient().GetAsync($"odata/Tasks({id})?$expand=BasePayrollRate,BaseServiceRate");
            response.EnsureSuccessStatusCode();

            using var responseContent = await response.Content.ReadAsStreamAsync();
            return await JsonSerializer.DeserializeAsync<Brizbee.Dashboard.Wasm.Models.Task>(responseContent);
        }

        public async Task<(List<Brizbee.Dashboard.Wasm.Models.Task>, long?)> GetTasksForPunchesAsync(DateTime min, DateTime max)
        {
            ...
            if (!response.IsSuccessStatusCode)
                return (new List<Brizbee.Dashboard.Wasm.Models.Task>(0), 0);

            using var responseContent = await response.Content.ReadAsStreamAsync();
            var value = await JsonSerializer.DeserializeAsync<List<Brizbee.Dashboard.Wasm.Models.Task>>(responseContent, options);
            var total = long.Parse(response.Headers.GetValues("X-Paging-TotalRecordCount").FirstOrDefault());
            return (value, total);
        }

        public async Task<Brizbee.Dashboard.Wasm.Models.Task> SearchTasksAsync(string number)
        {
            ...
            {
                using var responseContent = await response.Content.ReadAsStreamAsync();
                var odataResponse = await JsonSerializer.DeserializeAsync<Brizbee.Dashboard.Wasm.Models.Task>(responseContent, options);
        ...
        public async Task<Brizbee.Dashboard.Wasm.Models.Task> SaveTaskAsync(Brizbee.Dashboard.Wasm.Models.Task task)
        {
                ...
                using (var stringContent = new StringContent(json, Encoding.UTF8, "application/json"))
                {
                        ...
                        .ConfigureAwait(false))
                    {
                        if (response.IsSuccessStatusCode)
                        {
                            ...
                            {
                                var deserialized = await JsonSerializer.DeserializeAsync<Brizbee.Dashboard.Wasm.Models.Task>(responseContent, options);
                                return deserialized;
                            }
                        }
                        else
