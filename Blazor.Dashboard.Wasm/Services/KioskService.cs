...
namespace Brizbee.Dashboard.Wasm.Services
{
    public class KioskService
    {
        ...
        public async Task<Brizbee.Dashboard.Wasm.Models.Task> SearchTasksAsync(string taskNumber)
        {
            ...
            {
                using var responseContent = await response.Content.ReadAsStreamAsync();
                var value = await JsonSerializer.DeserializeAsync<Brizbee.Dashboard.Wasm.Models.Task>(responseContent, options);
                return value;
            }
            else
            {
        ...
        public async Task<List<Brizbee.Dashboard.Wasm.Models.Task>> GetTasksAsync(int projectId)
        {
            var response = await _apiService.GetHttpClient().GetAsync($"api/Kiosk/Tasks?projectId={projectId}");

            if (!response.IsSuccessStatusCode)
                return new List<Brizbee.Dashboard.Wasm.Models.Task>(0);
