...
namespace Brizbee.Common.Repositories
{
    public class PunchRepository
    {
            ...
            string sourcePhoneNumber = "")
        {
            ...
            var existing = _context.Punches.Where(p => p.UserId == currentUser.Id)
        ...
        private void AuditPunch(int id, string before, string after, User currentUser, string action)
        {
            try
            {
                ...
                var result = _context.Database.GetDbConnection().Execute(insertQuery, new
                {
